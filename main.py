...
def simple_demo():
    """Простая демонстрация для быстрого старта."""
    print("\n" + "="*80)
...
def main():
    ...
    while True:
        print("\n" + "="*80)
        print("🔌 DC CIRCUIT ANALYSIS ENVIRONMENT")
        print("="*80)
        print("\nВыберите действие:")
        print("  1. 🚀 Простая демонстрация (быстрый старт)")
        print("  2. 🔌 Демонстрация генерации задач")
        print("  3. ✅ Демонстрация верификации")
        print("  4. 🔍 Валидация системы")
        print("  5. 🎓 Запуск RL обучения")
        print("  6. ❌ Выход")
        ...
        if choice == "1":
            ...
            print("\n" + "="*80)
            print("🎓 ЗАПУСК RL ОБУЧЕНИЯ")
            print("="*80)
            print("\nДля запуска обучения используйте:")
            print("  python training/rl_trainer.py")
            print("\nИли в коде:")
            print("  from training.rl_trainer import DCCircuitRLTrainer, TrainingConfig")
            print("  config = TrainingConfig(max_steps=500)")
            print("  trainer = DCCircuitRLTrainer(config)")
            print("  trainer.run()")
        elif choice == "6":
            print("\n👋 До свидания!")
            break
